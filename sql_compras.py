...
# =====================================================================
# FUNCIONES PARA DASHBOARD (FALTANTES)
# =====================================================================

def get_dashboard_totales(anio: int) -> dict:
    """Totales generales para el dashboard"""
    ...
    return {"facturas": 0, "proveedores": 0, "total_pesos": 0.0, "total_usd": 0.0}


def get_dashboard_compras_por_mes(anio: int) -> pd.DataFrame:
    """Compras agrupadas por mes para gráfico"""
    total_expr = _sql_total_num_expr_general()
    sql = f"""
        SELECT
            TRIM("Mes") AS Mes,
...
def get_dashboard_top_proveedores(anio: int, limite: int = 10, moneda: str = "$") -> pd.DataFrame:
    ...
    moneda_filter = ""
    if moneda == "$":
        moneda_filter = "AND TRIM(\"Moneda\") = '$'"
    elif moneda in ("U$S", "USD"):
        moneda_filter = "AND TRIM(\"Moneda\") IN ('U$S', 'U$$', 'USD')"
...
def get_dashboard_gastos_familia(anio: int) -> pd.DataFrame:
    """Gastos agrupados por familia de productos"""
    total_expr = _sql_total_num_expr_general()
    sql = f"""
        SELECT
            COALESCE(TRIM("Familia"), 'Sin Familia') AS Familia,
            COALESCE(SUM({total_expr}), 0) AS Total
        FROM chatbot_raw
        WHERE ("Tipo Comprobante" = 'Compra Contado' OR "Tipo Comprobante" LIKE 'Compra%%')
          AND "Año" = %s
