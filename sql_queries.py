...
def get_comparacion_proveedor_anios_monedas(anios: List[int], proveedores: List[str] = None) -> pd.DataFrame:
    ...
    query = f"""
        ...
        FROM {TABLE_COMPRAS}
        WHERE ({COL_TIPO_COMP} = 'Compra Contado' OR {COL_TIPO_COMP} LIKE 'Compra%')
          AND {COL_ANIO} IN ({", ".join(str(y) for y in anios)})
          {prov_where}
        GROUP BY TRIM({COL_PROV})
        ORDER BY {order_sql}
        LIMIT 300
...
def get_compras_por_mes_excel(mes_key: str) -> pd.DataFrame:
    ...
    query = f"""
        ...
        WHERE TRIM(COALESCE({COL_MES}, '')) = %s
          AND ({COL_TIPO_COMP} = 'Compra Contado' OR {COL_TIPO_COMP} LIKE 'Compra%')
        ORDER BY {fecha_expr} DESC NULLS LAST, TRIM({COL_PROV}), TRIM({COL_ART})
    """
...
def buscar_comprobantes(proveedor=None, tipo_comprobante=None, articulo=None,
    ...
    if texto_busqueda and texto_busqueda.strip():
        sql += f""" AND (
            ...
            OR LOWER(TRIM({COL_ART})) LIKE LOWER(%s)
            OR LOWER(TRIM({COL_PROV})) LIKE LOWER(%s)
        )"""
        texto = f"%{texto_busqueda.strip()}%"
        params.extend([texto, texto, texto])

    sql += f" ORDER BY {fecha_expr} DESC NULLS LAST LIMIT 200"
