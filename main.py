...
def procesar_pregunta(pregunta: str) -> Tuple[str, Optional[pd.DataFrame]]:
    ...
    # --- PRIORIDAD 8: DETALLE COMPRAS PROVEEDOR + MES ---
        ...
        if 'Total' in df.columns:
            try:
                total = df['Total'].apply(lambda x: float(x) if pd.notna(x) else 0).sum()
            except:
                pass

        total_fmt = f"${total:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
        return f"📋 Compras de {proveedor_like.upper()} en {mes_key} | 💰 **Total: {total_fmt}** | {len(df)} registros:", formatear_dataframe(df)
